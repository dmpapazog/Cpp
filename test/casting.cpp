...
{
    public:
        BaseException(const string& leftName, const string& rightName, const char* action)
            : runtime_error("")
        {   
            stringstream oss;

            oss << "Can't " << action << " to \"" << leftName << "\" from \"" << rightName << "\". "
                << "\"" << rightName << "\" is empty.\n";
...
class Base {
    ...
    public:
        ...
        explicit Base(const string& t_name)
            : m_name(t_name)
        {
        ...
        Base(const Base& old, const string& t_name)
            : m_name(t_name)
        {
            if (old.m_x == nullptr) {
        ...
        Base(Base&& old, const string& t_name)
            : m_name(t_name)
        {
            if (old.m_x == nullptr) {
